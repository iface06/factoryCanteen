package de.vawi.fileManagement;
...
/**
 * Die Klasse Datei soll in einfacherer Weise Strings als Zeilen ausgeben, bzw
 * einlesen koennen.
 *
 ...
 */
public class VawiFileManager implements FileReader {
    ...
    /**
     * Enthaelt nach dem Aufruf einer Methode einen Fehlercode. 0 bedeutet, dass
     * kein Fehler aufgetreten ist und ueber die Methode errorMessage kann man
     * eine Beschreibung eines Fehlercodes erfragen.
     */
    ...
    /**
     * Konstruktor fuer Objekte der Klasse Datei Legt einen String mit dem Namen
     * der zu bearbeitenden Datei an.
     *
     * @param in_name (String): Dateiname der benutzt werden soll.
     */
    public VawiFileManager(String in_name) {
     ...
     *
     * @return gibt true nur zurueck, wenn das Ende einer Eingabedatei erreicht
     * wurde. Ansosnsten wird immer false zurueckgegeben.
     */
    public boolean eof() {
        return eof;
    }
     ...
     *
     * @param in_str - (String)Zeichenkette die in die Datei geschrieben werden
     * soll.
     */
    ...
    /**
     * Schreibt einen String als Zeile in eine Datei. (Variante mit
     * Fehlercodes!)
     *
     * @param in_str - (String)Zeichenkette die in die Datei geschrieben werden
     * soll.
    ...
    public int openInFile_FS() {
        try {
            dEin = new BufferedReader(new java.io.FileReader(dateiName));
            errorCode = 0;
            eof = false;
            return 0;
        } catch (Exception e) {
    ...
    /**
     * Liest eine Zeile aus einer Eingabedatei. (Variante mit Fehlercodes!) Hier
     * bitte darauf achten, das die Methode keinen Fehlercode zurueck gibt,
     * sondern -falls ein Fehler passiert) diesen nur ins Attribut errorCode
     * schreibt.
     *
     * @return String - ausgelesene Zeile.
     */
    ...
    /**
     * Gibt den aktuellen Zustand des Dateiobjektes zurueck. "true" bedeutet es
     * sind keine Fehler aufgetreten, "false" bedeutet es sind Fehler
     * aufgetreten, die ueber den Fehlercode in errorCode genauer zu
     * identifizieren sind. Diese Methode ist -mit einer Ausnahme- nur bei
     * Nutzung des integrierten Exceptionhandlings (<Methodenname>_FS) sinnvoll
     * nutzbar. Allein die Methode "deleteFile()" schreibt einen Errorcode (fuer
     * den Fall das die zu loeschende Datei keine Datei ist).
     *
    ...
    public String errorMessage(int in_error) {
        ...
        switch (in_error) {
            case 0:
                return "Ok!";
            case 1:
                return "(E) oeffnen des Ausgabefiles gescheitert.";
            case 2:
                return "(E) Schreiben einer Zeile gescheitert.";
            case 3:
                return "(E) Schliessen des Ausgabefiles gescheitert.";
            case 4:
                return "(E) oeffnen des Eingabefiles gescheitert.";
            case 5:
                return "(E) Lesen einer Zeile gescheitert.";
            case 6:
                return "(E) Schliessen des Eingabefiles gescheitert.";
            case 7:
                return "(E) Konnte Datei nicht loeschen.";
            case 8:
                return "(E) Konnte keine Datei waehlen.";
            case 98:
                return "(W) Es koennen nur Dateien geloescht werden.";
            default:
                return "(E) Nicht bekannter Fehler!";
        }
    }

    protected BufferedReader createBufferedReader() throws FileNotFoundException {
        return new BufferedReader(new java.io.FileReader(dateiName));
    }

    protected PrintWriter createPrintWriter() throws IOException {
        return new PrintWriter(new BufferedWriter(new java.io.FileWriter(dateiName)));
    }
}
